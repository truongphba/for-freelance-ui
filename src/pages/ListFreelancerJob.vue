<template>
  <div>
    <h3 class="font-owsald">My Freelancer Jobs</h3>
    <div v-if="this.user.freelancerDTO !== null">
      <q-tabs
        v-model="tab"
        class="text-green-7 q-mb-md"
      >
    ...
    <div v-else>
      <p class="title">You are not freelancer yet! Register to become freelancer</p>
      <q-btn label="Become Freelance" outline rounded type="submit" size="20px" color="white"
             class="q-px-lg bg-yellow-9" to="/register-freelancer"/>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    loadJobs () {
      axios.get(process.env.SOURCE_URL + '/job/list?freelancerId=' + this.user.freelancerDTO.id).then(res => {
  ...
  mounted () {
    if (this.user.freelancerDTO !== null) {
      this.loadJobs()
    }
  }
}
